...
export function ResumeSection() {
    ...
    const hasEducation = education && Object.keys(education).length > 0;
    const hasAwards = awards && awards.length > 0;
    const hasSkills = skills && skills.length > 0;

    if (!hasExperience && !hasEducation && !hasAwards && !hasSkills) {
        return null;
    }
                ...
                <div className="space-y-12">
                    {hasEducation && (
                        <div>
                            ...
                            >
                                {education.school && <h4 className="text-[20px] font-semibold text-foreground mb-2">{education.school}</h4>}
                                {education.date && <p className="text-sm text-muted font-mono mb-6">{education.date}</p>}
